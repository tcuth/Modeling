...
def run_wrapper(params):
	...
	if score < best_score:
		best_score = score
		best['alpha'], best['l1_ratio'], best['eta'] = alpha, l1_ratio, eta
...
if MAX_EVALS:
	...
	with open('/data/bin/modeling/moe.sgd.config.json','r') as f:
	...
	for i in range(MAX_EVALS):
		...
		alpha, l1_ratio, eta = json.loads(r.text)['points_to_sample'][0]
		alpha = float(alpha)
		l1_ratio = float(l1_ratio)
		eta = float(eta)
	
		hyperparameters = (alpha, l1_ratio, eta)
		scores = run_wrapper(hyperparameters)
		score, score_sd = np.mean(scores), np.std(scores)
		update_point = {"point":list(hyperparameters),\
						"value":score,\
						"value_var":score_sd **2}
...
#			best['alpha'], best['l1_ratio'], best['eta'] = alpha, l1_ratio, eta
	...
	print best
	print "Score for best hyperparameters: %f.4" % best_score
