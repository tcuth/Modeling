...
"""
Gradient Boosing Classifier tuned with hyperopt.
Uses creditR::ksC for objective function evaluations.
Requires xgb conda environmnet (openblas/lapack issue).
"""
...
def absKS(tag, score, weights):
	if w is not None:
		ks = creditR.ksC(score, tag, weights)
	else:
		ks = creditR.ksC(score, tag)
	return -1.0 * z[0] 
...
def cv_score(estimator, X, y, weights,n_jobs,k,metric):
	...
	scores = parallel(delayed(train_one_fold)(clone(estimator), X, y, weights, train_idx, test_idx,metric) \
...
def run_test(params):
	...
	gbt = GradientBoostingClassifier(learning_rate = learning_rate, n_estimators = n_estimators, \
			...
			loss = 'deviance', max_features = "auto", verbose = 1.0, random_state = SEED)
...
#			)
	space = (hp.loguniform('learning_rate', log(1e-3), log(1)),\
			hp.qloguniform('n_estimators', log(100), log(2000), 1),\
			hp.quniform('max_depth', 4, 8, 1),\
			hp.uniform('subsample',0,1),\
			hp.quniform('min_samples_leaf', 1, 200, 1)\
			)
